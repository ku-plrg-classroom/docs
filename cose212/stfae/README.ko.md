...
`STFAE` 언어는 [`TFAE`](../tfae/README.ko.md) 언어에 서브타입 다형성(subtype
polymorphism)을 추가한 언어입니다. 이 과제에서는 `typeCheck`와 `interp` 함수를
구현합니다.

## `STFAE` 언어의 명세

`STFAE` 언어의 문법(syntax)과 의미가(semantics)가 궁금하면,
[`stfae-spec.pdf`](./stfae-spec.pdf) 참조하세요.
