...
`TRFAE` 언어는 [`RFAE`](../rfae/README.ko.md) 언어에 타입 시스템(type system)을
추가한 언어입니다. 이 과제에서는 `typeCheck`와 `interp` 함수를 구현합니다.

## `TRFAE` 언어의 명세

`TRFAE` 언어의 문법(syntax)과 의미가(semantics)가 궁금하면,
[`trfae-spec.pdf`](./trfae-spec.pdf) 참조하세요.
