...
```bash
sbt new ku-plrg-classroom/mini-scala.g8
```
...
└─ src
   ├─ main/scala/kuplrg
   │  ├── MiniScala.scala ────── The definition of the mini-scala and parsers
...
The `MiniScala` language is a toy language inspired by the [Scala programming
language](https://www.scala-lang.org/). The name `MiniScala` indicates that it
is a minimal subset of the Scala language.

## Specification of `MiniScala` language

See the [`mini-scala-spec.pdf`](./mini-scala-spec.pdf) for the syntax and
semantics of the `MiniScala` language.
