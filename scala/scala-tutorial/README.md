...
It takes an integer `init`, a function `f` whose type is `Int => Int`, and an
integer `n` and returns a list containing the first `n` elements generated by
repeatedly applying `f` to `init`. The first element should be `init`.
```scala
test(generate(1, _ + 3, 3), List(1, 4, 7))
