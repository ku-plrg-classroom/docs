...
- [**Pushdown Automata (PDA)**](#pushdown-automata-pda)
  ...
  - [(Problem #4) `pda_hamming_one_final` (10 points)](#problem-4-pda_hamming_one_final-10-points)
  - [(Problem #5) `pda_pal_concat_empty` (15 points)](#problem-5-pda_pal_concat_empty-15-points)
  - [(Problem #6) `pda_expr_x_odd_final` (15 points)](#problem-6-pda_expr_x_odd_final-15-points)
...
### (Problem #4) `pda_hamming_one_final` (10 points)
...
aab#bba         // because x^R = baa,  y = bba,  1 mismatch at position 2
aab#bab         // because x^R = baa,  y = bab,  1 mismatch at position 3
```
However, the following words are **not in the language**:
```plaintext
...
aab#abb         // because x^R = baa,  y = abb,  3 mismatches
...
### (Problem #6) `pda_expr_x_odd_final` (15 points)

Please implement the PDA `pda_expr_x_odd_final` whose language accepted by
**final states** is equal to the following language:

$${\large
L = \lbrace w \in \lbrace \texttt{x}, \texttt{+}, \texttt{(}, \texttt{)} \rbrace^* \mid
w \text{ is a valid arithmetic expression with an odd number of } \texttt{x} \rbrace
}$$

A **valid arithmetic expression** is a string generated by the following
grammar:
$$E \to \texttt{x} \mid \texttt{x++} \mid E\,\texttt{+}\,E \mid \texttt{(}\,E\,\texttt{)}$$

Each $\texttt{x}$ denotes a use of the variable $\texttt{x}$, $\texttt{x++}$
denotes a post-increment of the variable $\texttt{x}$, $E\,\texttt{+}\,E$
denotes an addition, and $\texttt{(}\,E\,\texttt{)}$ denotes a parenthesized
sub-expression. Both $\texttt{x}$ and $\texttt{x++}$ count as a single use of
the variable $\texttt{x}$, so the number of uses of $\texttt{x}$ in $w$ equals
the number of $\texttt{x}$ symbols in $w$.
...
x+x+x                   // 3 uses of x
x+x+x++                 // 3 uses of x
x+x+++x                 // 3 uses of x   (= x + x++ + x)
(x+x)+x                 // 3 uses of x
x+(x+x)                 // 3 uses of x
```
However, the following words are **not in the language**:
```plaintext
                        // because empty input is not an expression
+                       // because operator without operand
+x                      // because operator before operand
x+                      // because missing operand after operator
(x                      // because unbalanced parenthesis
x)                      // because unbalanced parenthesis
(x+)                    // because missing operand inside parentheses
()                      // because empty parentheses
xx                      // because no operator between operands
x++x                    // because no operator between operands
x+++                    // because incomplete expression   (= x++ + ?)
x+x                     // because 2 uses of x (even)
(x+x)                   // because 2 uses of x (even)
x+x+x+x                 // because 4 uses of x (even)
(x++)+(x++)             // because 2 uses of x (even)
```
...
represented by $w$ in binary. For example,
